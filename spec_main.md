...
# リアルタイム対話
bsdf dashboard --config config.yaml --port 5006      # Panel ダッシュボード起動（ローカルのみ）
bsdf dashboard --config config.yaml --host 0.0.0.0  # 他PCからもアクセス可能

# MLflow ブラウズ・可視化
bsdf runs list --sort-by haze_fft_0_t --limit 10     # run 一覧をメトリクス順で表示
...
ここで Rayleigh-Rice 理論の標準定義に従い、PSD は**散乱ベクトルの表面平行成分**で評価する:

$$f_x = \frac{\sin\theta_s \cos\phi_s - \sin\theta_i \cos\phi_i}{\lambda}, \quad f_y = \frac{\sin\theta_s \sin\phi_s - \sin\theta_i \sin\phi_i}{\lambda}$$

3. 空間周波数 $(f_x, f_y)$ と方向余弦 $(u, v) = (\sin\theta_s\cos\phi_s, \sin\theta_s\sin\phi_s)$ の対応は
   $u = f_x \lambda + \sin\theta_i \cos\phi_i$, $v = f_y \lambda + \sin\theta_i \sin\phi_i$ となり、**specular は $(\sin\theta_i\cos\phi_i, \sin\theta_i\sin\phi_i)$ に配置される**（FFT の `output_shift` モードと同じ規約）。フル半球カバレッジには $dx \leq \lambda/(2(1+\sin\theta_i))$ が必要で、これを超えると後方散乱側の一部が FFT 格子外に出て欠損する。
...
- **光学指標（BSDF・光伝搬から算出）**
  ...
  - 写像性（DOI / Clarity）: 3 方式を並列提供。詳細は 5.4 節参照。
    - **DOI-NSER**（Near-Specular Energy Ratio、独自方式・規格非準拠）: 直進光コーンと小角散乱コーンの BSDF エネルギー比。
    - **DOI-COMB**（JIS K 7374 光学くし方式、通称「くしば方式」）: 5 種のくし幅でのコントラスト $M(d)$ の平均。
    - **DOI-ASTM**（ASTM E430 Dorigon 方式）: $[R(0°) - R(0.3°)] / R(0°) \times 100$。
...
- **表面形状指標（高さ配列 $h(x,y)$ から算出）**
  ...
  **ISO 25178-2 S-パラメータ（面粗さ）**
  ...
  | $Sdr$ | 界面展開面積比 | $(\text{実面積}-\text{投影面積})/\text{投影面積}$（0〜1 の比率。規格値 [%] は ×100） |
...
sparkle:
  ...
  # 各波長で独立に Sparkle が計算され、`sparkle_<level>_<method>_<nm>_<deg>_<mode>`
  # 形式で記録される（例: `sparkle_l1_fft_525_0_t` / `sparkle_l5_fft_525_0_t`）。
  # `level` は `sparkle.level` で 'L1' / 'L3' / 'L4' / 'L5' から選択（既定 'L1'）。
...
| **DOI-NSER** | 規格非準拠（独自、最適化用） | — | — |
| **DOI-COMB** | **JIS K 7374**（Suga ICM / 光学くし） | 白色光源（A または C） | 明所視 V(λ) |
| **DOI-ASTM** | **ASTM E430**（Dorigon） | CIE Illuminant C | CIE 1931 Y |
...
| **Haze** | **なし** — 代表波長のみ | `haze_<method>_0_t`（θ_i=0, BTDF） |
| **Gloss** | **なし** — 代表波長のみ | `gloss_<method>_<20\|60\|85>_r`（規格 3 角、BRDF） |
| **DOI-NSER** | **なし** — 代表波長のみ | `doi_nser_<method>_0_t` |
| **DOI-COMB** | **なし** — 代表波長のみ | `doi_comb_<method>_0_<t\|r>`（BTDF/BRDF 両方） |
| **DOI-ASTM** | **なし** — 代表波長のみ | `doi_astm_<method>_<20\|30>_r`（ASTM E430 2 角） |
| **Sparkle** | **あり** — 各波長で独立計算 | `sparkle_<method>_<nm>_<deg>_<mode>` |
| **Log-RMSE** | **あり** — 各条件で独立計算 | `log_rmse_<method>_<nm>_<deg>_<mode>` |

命名規則: `<name>_<method>_<deg>_<mode>`（単波長メトリクス）/
`<name>_<method>_<nm>_<deg>_<mode>`（波長依存メトリクス）。
- `<method>` = `fft` / `psd` / `ml`
- `<deg>` = 入射角（整数）
- `<mode>` = `t`（BTDF）/ `r`（BRDF）
- `<nm>` = 波長 [nm]（整数）

規格外条件での評価は `metrics.compute_at_sim_angles: true` で有効化できる（デフォルト無効）。
...
### 5.4. 評価アルゴリズム詳細：写像性（DOI）3 方式

写像性（DOI / Distinctness of Image / Clarity / 写像鮮明度）は **3 方式を並列で提供**する。config の `doi_nser` / `doi_comb` / `doi_astm` セクションで個別に有効化でき、不要な方式はコメントアウトまたは `enabled: false` で無効化する。MLflow メトリック名はそれぞれ `doi_nser_<method>` / `doi_comb_<method>` / `doi_astm_<method>`（`<method>` は `fft` / `psd` / `ml`）。

#### 5.4.1. 方式 A：DOI-NSER（Near-Specular Energy Ratio、独自・規格非準拠）

直進光コーン（半角 $\alpha_d$、デフォルト 0.1°）と小角散乱コーン（半角 $\alpha_h$、デフォルト 2.0°）の BSDF エネルギー比：

$$\text{DOI}_{\text{NSER}} = \frac{\int_{|\theta| \le \alpha_d} \text{BSDF}(\theta) \cos\theta \, d\Omega}{\int_{|\theta| \le \alpha_h} \text{BSDF}(\theta) \cos\theta \, d\Omega} \in [0, 1]$$

**設計意図**: BSDF を直接使える最適化向けの簡易スカラー指標。光学系パラメータ（距離・絞り）不要、スカラー 1 値出力で Optuna 目的関数と親和的。

**注意**: 規格非準拠のため、実測 ICM（Suga 式）との直接比較には不適。実測整合が必要なら DOI-COMB を使う。

#### 5.4.2. 方式 B：DOI-COMB（JIS K 7374 光学くし方式・通称「くしば方式」）

JIS K 7374（Suga ICM 相当）に基づく光学くし方式を BSDF ベースで再現する。値域は **0〜1**（3 方式統一）。JIS K 7374 規格値（0〜100 [%]）が必要な場合は呼び出し側で ×100 する。

**物理モデル**:
- 試料 〜 光学くし間距離 $L$ = `distance_mm`（Suga ICM 標準 280 mm）
- くし幅 $d \in \{0.125, 0.25, 0.5, 1.0, 2.0\}$ mm（JIS K 7374 標準 5 種）
- 角度領域での明/暗 1 組の周期：$T(d) = 2d / L$ [rad]

**計算手順**:
1. BSDF を $v$ 軸の中心バンド（$|v| \le \sin(v\_band\_half\_deg)$）で平均し、1D 角度プロファイル $P(u)$ を得る。
2. 各くし幅 $d$ について、走査範囲 $\pm\sin(\text{scan\_half\_angle\_deg})$ 内で矩形波くし（duty 50%）を走査し、
   強度 $I(x) = \sum_{u: \text{bright}(u-x)} P(u) \, du$ を 32 位相サンプリング。
3. コントラスト $M(d) = (I_{\max} - I_{\min}) / (I_{\max} + I_{\min}) \in [0, 1]$。
4. $\text{DOI}_{\text{COMB}} = \frac{1}{5} \sum_d M(d) \in [0, 1]$（5 値の算術平均）。

**設計意図**: JIS K 7374 規格準拠、人間の映り込み知覚と相関が高い。Suga ICM 実測値との比較に使える。

**注意**: `distance_mm` と `comb_widths_mm` の組合せが BSDF グリッドの角度分解能を下回ると、エイリアシングで値が不安定化する。目安として **グリッド角度分解能 ≤ $T(d_{\min})/4$** を満たすこと（デフォルト設定では約 0.02° 分解能が必要）。

#### 5.4.3. 方式 C：DOI-ASTM（ASTM E430 Dorigon 方式）

ASTM E430（および ISO 2813 Annex）に準拠した 0°/オフセット角の反射率差分方式。値域は **0〜1**（3 方式統一）。ASTM E430 規格値（0〜100 [%]）が必要な場合は呼び出し側で ×100 する。

$$\text{DOI}_{\text{ASTM}} = \max\!\left(0, \frac{R(0°) - R(\alpha_{\text{off}})}{R(0°)}\right) \in [0, 1]$$

- $R(\theta)$: 中心または $\theta$ オフセット位置周りの BSDF·cosθ 積分値（絞り半角 `aperture_half_deg`、デフォルト 0.05°）
- $\alpha_{\text{off}}$ = `offset_deg`（ASTM 標準 0.3°）
- オフセットは $+u, -u$ 両方向の平均

**設計意図**: ASTM E430 規格準拠、スカラー 1 値、高光沢面（プラスチック塗膜等）の微細な分解能差を鋭敏に検出。

**注意**: AG フィルムのような中程度の散乱では $R(0°) \approx R(0.3°)$ となり感度が飽和して 0 に張り付く傾向がある。AG 設計の最適化指標としては DOI-NSER または DOI-COMB が向く。

#### 5.4.4. 三方式の使い分けガイド

| 目的 | 推奨方式 |
|---|---|
| Optuna 最適化の目的関数（軽量・安定） | **DOI-NSER** |
| Suga ICM（JIS K 7374）実測値との比較 | **DOI-COMB** |
| 高光沢面の規格値比較（ASTM E430 規格） | **DOI-ASTM** |
| AG フィルム開発全般 | **DOI-NSER** + **DOI-COMB**（併用） |

---

### 5.5. 評価アルゴリズム詳細：Gloss 3 角並列と黒ガラス基準化

Gloss は規格標準 3 角（20°/60°/85°）を並列出力する。各角度の長方形絞り
（ISO 2813）は:

| 角度 | 受光絞り（in-plane × cross-plane） |
|---|---|
| 20° | 1.8° × 3.6° |
| 60° | 4.4° × 11.7° |
| 85° | 4.0° × 6.0° |

**黒ガラス基準化** (`black_glass_normalization: true`):

$$\text{GU} = 100 \times \frac{F_{\text{sample}}(\theta)}{R_{\text{BG}}(\theta)}$$

- $R_{\text{BG}}(\theta)$ = 屈折率 $n=1.567$（規格固定値、波長非依存扱い）の Fresnel 反射率（無偏光）
- 黒ガラスの「黒」は透過光完全吸収（背面反射排除）を意味するが、本 sim の BRDF 計算は
  既に Kirchhoff スカラー回折で **前面反射のみ**をモデル化しているため、$n$ の情報だけで
  基準値 $R_{\text{BG}}(\theta)$ が得られる（多層吸収モデル不要）。
- 実ユーザーがサンプルに**透明フィルム**を使う場合、実測 Gloss は背面反射込みになるため
  sim 値と系統差が出る点に注意。
...
- **リアルタイム・ダッシュボード（DynamicMap）**:
  - `--host 0.0.0.0` オプションで他 PC からアクセス可能（ファイアウォールでポート開放が必要）。デフォルトは `localhost`（ローカルのみ）。WebSocket オリジン制限は `address=0.0.0.0` 時に自動で `*` に緩和される。
...
| `test_doi_nser_range` | DOI-NSER ∈ [0, 1] |
| `test_doi_nser_narrow_beam` | 法線集中ビームの DOI-NSER > 0.5 |
| `test_doi_comb_range` | DOI-COMB ∈ [0, 1] |
| `test_doi_comb_narrow_beam_high_contrast` | 狭幅ピークで DOI-COMB > 0.1 |
| `test_doi_comb_uniform_low_contrast` | 一様 BSDF で DOI-COMB < 0.05 |
| `test_doi_astm_range` | DOI-ASTM ∈ [0, 1] |
| `test_doi_astm_narrow_beam` | 狭幅ピークで DOI-ASTM > 0.9 |
| `test_doi_astm_wide_beam_low` | 広幅散乱で DOI-ASTM < 0.1 |
...
#### [BUG-008] `bsdf dashboard --host 0.0.0.0` でブラウザが到達不能エラー — 修正済み
- **発見日**: 2026-04-17（ユーザー実行時）
- **影響ファイル**: `src/bsdf_sim/visualization/dynamicmap.py`
- **症状**: `bsdf dashboard --host 0.0.0.0` で起動すると自動起動したブラウザに "このページに到達できません" と表示される。
- **原因**: `pn.serve(address='0.0.0.0', show=True)` はバインドアドレスをそのままブラウザ URL として使うが、`0.0.0.0` はリッスン専用アドレスで Chrome 等のモダンブラウザはクライアント側で `http://0.0.0.0:PORT` を解決できない。
- **修正**: `serve()` で `address='0.0.0.0'` かつ `show=True` の場合は `pn.serve` の `show=False` にし、`threading.Timer` で 1.5 秒後に `http://localhost:PORT` を手動で開くよう変更。サーバーは 0.0.0.0 にバインドしたままなので外部 PC からのアクセスは維持される。
- **対応コミット**: `583c710`

#### [BUG-009] dashboard 1D プロファイルが青線で二重描画される — 修正済み
- **発見日**: 2026-04-17（ユーザー実行時）
- **影響ファイル**: `src/bsdf_sim/visualization/dynamicmap.py`
- **症状**: `bsdf dashboard` で 1 モデルのみ指定しているにもかかわらず、FFT 計算の青線が折り返すような二重曲線として描画される。実測 BSDF が表示されない（実際にはプロットが重なって視認しづらい）ケースとセットで発生。
- **原因**: `_make_1d_overlay()` が `u[:, 0]` を `np.abs()` してから `hv.Curve` に渡していた。`np.fft.fftfreq(N)` の並びは `[0, +1/N, ..., +0.5, -0.5, ..., -1/N]` のため、`abs` を取ると `0 → 0.5 → 0` を往復する配列になり、Curve がこれを順序通り結んで往復する見た目になる。往復の正負側で数値がわずかに異なる条件では「二重線」として視認される。
- **修正**: `u >= 0` の半分だけを抽出し `argsort` で theta_s 昇順に並べてから Curve を作るように変更。phi=0 プロファイルは u 軸対称なので半分で十分。
- **対応コミット**: 本コミット

#### [BUG-010] PSD 法の u/v グリッドに sin θ_i オフセットが無かった — 修正済み
- **発見日**: 2026-04-18（FFT vs PSD 比較プロット検討中にユーザー指摘）
- **影響ファイル**: `src/bsdf_sim/optics/psd_bsdf.py`
- **症状**: `compute_bsdf_psd()` の出力 BSDF グリッドで、θ_i > 0° でもスペキュラが常に `(u, v) = (0, 0)`（θ_s=0°）に留まる。入射角を変えても BSDF の角度分布形状が θ_i 非依存に見え、`cos(θ_i)` と `Q(θ_i, θ_s)` による縦方向スケールのみ変化していた。FFT 法（tilt / output_shift）と比較すると明らかに形状が一致しない。
- **原因**: `u_grid = fx * λ`, `v_grid = fy * λ` で `fx, fy = np.fft.fftfreq(...)` をそのまま方向余弦に対応付けていた。Rayleigh-Rice 理論の標準定義は `f_x = (sin θ_s cos φ_s − sin θ_i cos φ_i) / λ`（Stover, Church & Takacs, Elson-Bennett）であり、`u = sin θ_s cos φ_s = f_x · λ + sin θ_i cos φ_i` と `sin θ_i` オフセットを加える必要がある。θ_i = 0 では偶然一致するため正常動作しているように見え、見落とされていた。
- **修正**: `u_grid = fx * λ + sin θ_i · cos φ_i`, `v_grid = fy * λ + sin θ_i · sin φ_i` に変更（FFT の `output_shift` モードと同じ座標規約）。フル半球カバレッジ条件 `dx ≤ λ / (2·(1 + sin θ_i))` を docstring に明記。θ_i = 0 の既存テストは数値不変、θ_i > 0 のテストは当時 PSD 法の絶対値のみ検証していたため全 331 件 pass。
- **対応コミット**: 本コミット
...
| 2026-04-18 | **BSDF 1D プロットに副軸（上段 X 軸）を追加**: 散乱角 θ_s [deg] を主軸とする 1D プロットに、**波動ベクトル系のアナロジー**（θ_s と単調関数で結ばれる量）を副軸で表示可能に。`visualization/secondary_axis.py` を新設し 5 種のユニットを定義: `theta_s`（副軸なし）/ `lambda_scale`（構造スケール Λ=λ/sin θ_s [μm]、**既定**）/ `u`（方向余弦）/ `f`（空間周波数 [μm⁻¹]）/ `k_x`（波数 [rad/μm]）。dashboard 3 種の `_make_axis_controls` に副軸ユニット Select を追加、visualize レポート（`plot_bsdf_1d_overlay` / `create_scale_toggle_panel` / `plot_bsdf_report`）と `_demo_overlay.py` デモにも伝播。Bokeh は `FuncTickFormatter` で JS 側 θ_s→副軸値変換、matplotlib は `ax.twiny()` + 明示範囲設定（`secondary_xaxis` は forward 関数が monotonic-decreasing の log 軸で左右 swap する問題があるため twiny で回避）。副軸ラベルは日本語（Bokeh）と英語（matplotlib tofu 回避）の両方を保持。テスト 381→398 件（+17） | 本コミット |
| 2026-04-18 | **BSDF 指標オーバーレイ Phase 3 描画詳細テスト**（テスト追加のみ、挙動変更なし）: `tests/test_metric_overlays.py` に `TestOverlayGeometry` クラスを追加し、各 overlay の座標・寸法を数値検証。(a) Haze 円半径 = sin(half_angle_deg)、(b) Gloss 長方形の中心 u=sin(angle) と du_half=cos(angle)·rad(ap/2) / dv_half=rad(cross/2)、(c) NSER 内外円の半径、(d) COMB band 長方形と pitch バー周期 period_u=2·d_min/distance、(e) ASTM 3 円の位置 (0, ±sin(offset)) と絞り直径、(f) θ_i=30° BRDF での中心シフト の 9 ケース。テスト 372→381 件 | 本コミット |
| 2026-04-18 | **X 軸 log/linear 切替を visualize レポート・デモに横展開**: 従来 dashboard のみに存在した X 軸スケール切替機能を次の 4 箇所に適用 — (A) `plot_bsdf_1d_overlay` に `xscale="linear"` 引数追加（`log` 時は `theta_s > 0.05°` にフィルタし `xlim=(0.1, 90)`）、(B) `create_scale_toggle_panel` に X 軸 RadioButtonGroup 追加（Y 軸セレクタと並置）、(C) `plot_bsdf_report` / `_build_condition_panel` に `xscale` 引数を伝播し多条件 Tab でも切替可能に、(D) matplotlib デモ 4 本に `XSCALE` / `YSCALE` モジュール定数を公開（`_demo_overlay.py` のみ X-log 対応、他 3 本は signed x のため linear 固定を明記）。テスト 372 件 pass | 本コミット |
| 2026-04-18 | **BSDF 指標オーバーレイ Phase 2 拡張**（追加機能、後方互換）: `visualization/metric_overlays.py` に (1) `overlay_doi_comb_2d`（走査帯 ±sin(scan_half)×sin(v_band) の長方形 + 最小くし幅の pitch バー）、(2) `overlay_doi_astm_2d`（0° + ±sin(offset) の 3 円）、(3) `overlay_from_config`（config.yaml ルート辞書から `visualization.metric_overlay` を読んで適用）を追加。`overlay_all_metrics_2d` に COMB/ASTM 統合と `initially_shown` リストに基づくレイヤー別 `visible=False` 適用を実装。`config.yaml` に `visualization.metric_overlay` セクション（`show_overlay` / `initially_shown` / `click_policy` / `legend_position`）を追加。サンプル PNG (`outputs/demo_metric_overlays.png` + zoom) を出力するデモスクリプト `outputs/_demo_metric_overlays.py` を追加。テスト 364→372 件（COMB/ASTM/initially_shown/from_config 計 8 件追加） | 本コミット |
| 2026-04-18 | **1D プロットの軸ラベル定数化**（内部構造のみ、挙動変更なし）: `visualization/constants.py` に `AXIS_THETA_S_LABEL="散乱角 θ_s [deg]"` と `AXIS_BSDF_LABEL="BSDF [sr⁻¹]"` を追加。dashboard `_make_1d_overlay` と visualize `plot_bsdf_1d_overlay` + `plot_bsdf_2d_heatmap` colorbar で計 10 箇所に直値で書かれていた文字列を定数化。`_make_1d_overlay`（grid 入力）と `plot_bsdf_1d_overlay`（DataFrame 入力）の本体統合は入出力型が根本的に違うため見送り（#6 検討結果、B 案の軽い定数化のみ採用）。テスト 364 件 pass | 本コミット |
| 2026-04-18 | **実測プロファイル抽出の軽い共通化**（内部構造のみ、挙動変更なし）: `visualization/profile_extract.py` に `sort_and_floor(df_sub)` ヘルパーを追加し、dashboard の `_extract_measured_profile` と visualize レポートの `plot_bsdf_1d_overlay` の method ループで重複していた「`theta_s_deg` 昇順ソート + BSDF floor クリップ」の 3 行パターンを単一実装に集約。併せて `visualization/constants.py` に `MEASURED_PHI_S_TOL_DEG=6.0` を追加し、phi_s≈0 プロファイルと見なす方位角許容幅（両所で 6.0 をハードコード）を定数化。フル統合（C 案）は入力前提（pre-filter 済み vs フル DataFrame）が違うため推奨しないと判断し、軽い共通化（B 案）を採用。テスト 345→364 件 | 本コミット |
| 2026-04-18 | **BSDF 可視化共通化リファクタ**（内部構造のみ、挙動変更なし）: (1) `visualization/profile_extract.py::slice_phi0()` を新設し、phi≈0 プロファイル抽出（fftfreq 順序対応・u 正半分 or 符号付き・v 帯平均・floor クリップ）を集約。dashboard `_make_1d_overlay` と 4 つのデモスクリプトで重複していた同種ロジックを単一実装に統一（BUG-009 の対策を全体ポリシー化）。(2) `visualization/constants.py::BSDF_LOG_FLOOR_DEFAULT=1e-10` を定数化し、log スケール下限クリップ値の散在（`1e-10` が 5 箇所・`1e-12` が 1 箇所）を集約。`config.error_metrics.bsdf_floor=1e-6`（Log-RMSE 用）とは役割が異なるため定数名で明示分離。(3) `_BaseBSDFDashboard._make_axis_controls()` を追加、Y/X スケール切替と Y 範囲固定 UI（5 ウィジェット）を 3 ダッシュボード（RandomRough/Spherical/Measured）で重複していた定義を 1 呼び出しに統合。テスト 337 件（`test_profile_extract.py` +6 件） | 本コミット |
| 2026-04-18 | `compute_bsdf_fft` に `apply_fresnel` 引数追加（既定 `False`、後方互換）: True 時に BRDF/BTDF の絶対値に `R(θ_i)=(|r_s|²+|r_p|²)/2`（Unpolarized）/ `T(θ_i)=(n2 cos θ_t)/(n1 cos θ_i)·(|t_s|²+|t_p|²)/2` を後掛けし、スカラー近似で失われる θ_i 依存の反射/透過率を回復する。`config.fft.apply_fresnel` で CLI/dashboard からも有効化可。θ_s 依存性は含まないヒューリスティックなので、厳密性が必要な場合は PSD 法（Rayleigh-Rice）を使う。BUG-010（PSD の sin θ_i オフセット修正）と合わせて実装 | 本コミット |
| 2026-04-17 | **DOI 3 方式化（破壊的変更）**: 旧 `compute_doi` / `doi` キー / `doi_fft` メトリックを廃止し、3 方式を並列提供: `doi_nser`（現行の NSER、Near-Specular Energy Ratio、独自・規格非準拠）/ `doi_comb`（JIS K 7374 光学くし方式・通称「くしば方式」、5 くし幅 [0.125,0.25,0.5,1.0,2.0] mm のコントラスト平均）/ `doi_astm`（ASTM E430 Dorigon 方式、0°/0.3° 反射率差分比）。config は `doi_nser`/`doi_comb`/`doi_astm` セクションで個別制御、MLflow メトリックは `doi_{nser,comb,astm}_fft`。**3 方式とも値域を `[0, 1]` に統一**（JIS K 7374 / ASTM E430 の規格値 `[0, 100]` [%] が必要な場合は呼び出し側で ×100）。spec Section 5.4 に詳細追加。テスト 311→330 件 | 本コミット |
| 2026-04-17 | **Sdr 値域統一（破壊的変更）**: `compute_sdr` の戻り値を 0〜100 [%] から **0〜1 の比率**に変更（DOI と値域統一）。メトリックキー `sdr_pct` → `sdr` にリネーム（`_pct` サフィックスが実態と乖離するため）。ISO 25178-2 規格値 [%] が必要な場合は呼び出し側で ×100（dashboard 表示は従来通り `%` サフィックス付き）。影響: MLflow メトリック名、`compute_all_surface_metrics` 戻り値キー、dashboard 表示コード | 本コミット |
| 2026-04-18 | **規格 θ_i 自動追加 sim + メトリクス命名統一（破壊的変更）**: Haze/Gloss/DOI の規格条件 (θ_i, mode) に対応する BSDF を自動追加 sim で計算し、sim の `theta_i_deg` に関係なく規格値を出力する。Gloss は 20°/60°/85° 並列、DOI-COMB は BTDF/BRDF 両方、DOI-ASTM は 20°/30° 並列。Gloss に規格準拠の長方形絞り（ISO 2813）と黒ガラス基準化オプション（n=1.567、GU 値出力）を追加。斜入射対応値は `metrics.compute_at_sim_angles: true` で有効化（デフォルト無効）。**メトリクス命名を `<name>_<method>_<deg>_<mode>` 形式に統一**（例: `haze_fft_0_t`, `gloss_fft_60_r`, `doi_astm_fft_20_r`）。波長依存メトリクス（Sparkle/Log-RMSE）は `<name>_<method>_<nm>_<deg>_<mode>` 形式（旧 `wl525nm_aoi30_brdf` → `525_30_r`）。テスト 330→331 件 | 本コミット |
| 2026-04-17 | `compute_bsdf_fft` に `fft_mode` 引数追加（`tilt`/`output_shift`/`zero`）: `tilt`（既定、現行挙動） / `output_shift`（位相傾き項を入力に入れず出力 u_grid をシフト、スペクトル漏れなしだが θ_i 大で後方散乱欠損） / `zero`（垂直入射近似、θ_i 非依存で 1 回計算）。詳細は `docs/fft_bsdf_math.md` 参照。テスト 311→319 件 | 本コミット |
| 2026-04-17 | dashboard X 軸リニア／対数切替 + surface.png 表示サイズ縮小: `_make_1d_overlay()` に `xscale` 引数を追加し 3 ダッシュボード全てに `xscale_selector`（RadioButtonGroup）を配置（log 時は `theta>0.05` のみ表示）。`save_heightmap_png()` の figsize を (12,9) → (6,4.5) に半減、フォントサイズも調整して MLflow UI でスクロール不要に | `c699d27` |
...
未実装の機能・改善項目・既知の不具合はリポジトリルートの **[TODO.md](TODO.md)** に集約している。

- 🚨 緊急（バグ修正）
- ✨ 改善・機能追加（仕様書で言及されている未実装機能・光学指標規格対応・Dashboard 機能拡張・CLI/UI 拡張・テスト/品質）
- 💡 将来的なアイデア

実装が完了した項目は TODO.md から削除し、**Section 10.2 の仕様変更履歴**テーブルに移動する。新規発生した未着手課題は TODO.md に追記する。
