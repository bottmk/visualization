...
**UI 機能**:
- **Y 軸（BSDF）スケール**: linear / log をラジオボタンで切替
- **X 軸（角度）スケール**: linear / log をラジオボタンで切替（log 時は θ > 0.05° のみ表示）
- **Y 軸範囲を固定**: チェックボックス ON で Y最小 / Y最大 を手動指定可（OFF 時は自動スケール）
- **X 軸目盛**: linear 時は 0-90° 固定、主目盛 10°刻み / 補助目盛 5°刻み（log 時は bokeh LogTicker 自動）
