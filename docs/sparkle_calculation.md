...
**本実装での注意**: 現行の L1 実装（`compute_sparkle`）が返す値は角度ビニング手法の性質上、実測値より 1–2 桁大きい **apparent $C_s$** である（DC 集中が dominant）。**相対比較・最適化目的関数** としては有用だが、規格値との直接比較は L5（`compute_sparkle_l5`）を推奨。詳細は `docs/sparkle_approximation_levels.md` 参照。
